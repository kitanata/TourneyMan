...
joint.dia.Graph = Backbone.Model.extend({
    ...
    getElements: function() {
      ...
      var self = this;
      return _.map(this._nodes, function(exists, node) { return self.getCell(node); });
    },

    getLinks: function() {

      var self = this;
      return _.map(this._edges, function(exists, edge) { return self.getCell(edge); });
    },
    ...
    getCommonAncestor: function(/* cells */) {
        ...
        var cellsAncestors = _.map(arguments, function(cell) {
            ...
            while (parentId) {
                ...
                parentId = self.getCell(parentId).get('parent');
            }
    ...
    getEmbeddedCells: function(opt) {
        ...
        if (this.graph) {
            ...
            if (opt.deep) {
                ...
                cells = _.map(this.get('embeds'), this.graph.getCell.bind(this.graph));
            }
...
joint.dia.ElementView = joint.dia.CellView.extend({
    ...
    update: function(cell, renderingOnlyAttrs) {
        ...
        _.each(renderingOnlyAttrs || allAttrs, function(attrs, selector) {
            ...
            self.updateAttr($selected, attrs);
        ...
        _.each(relativelyPositioned, function($el) {
            ...
            self.positionRelative(V($el[0]), bbox, elAttrs, nodesBySelector);
...
joint.dia.LinkView = joint.dia.CellView.extend({
    ...
    render: function() {
        ...
        _.each(children, function(child) {
            ...
            if (className) {
                ...
                self._V[$.camelCase(className)] = child;
            }
      ...
      var self = this;
        ...
        _.each(this.model.get('attrs'), function(attrs, selector) {
            ...
            if (_.isObject(attrs.fill)) {
                ...
                self.applyGradient(selector, 'fill', attrs.fill);
                processedAttributes.push('fill');
            }

            if (_.isObject(attrs.stroke)) {

                self.applyGradient(selector, 'stroke', attrs.stroke);
            ...
            if (_.isObject(attrs.filter)) {
                ...
                self.applyFilter(selector, attrs.filter);
            ...
            self.findBySelector(selector).attr(attrs);
    ...
    _markAvailableMagnets: function() {
        ...
        _.chain(elements).map(paper.findViewByModel, paper).each(function(view) {
            ...
            var availableMagnets = _.filter(magnets, _.partial(isMagnetAvailable, view).bind(this));
                    ...
                    _.each(viewsInArea, function(view) {
                        ...
                        if (view.el.getAttribute('magnet') !== 'false') {
                            ...
                            if (distance < r && distance < minDistance) {
                                ...
                                if (self.paper.options.validateConnection.apply(
                                    self.paper, self._validateConnectionArgs(view, null)
                                )) {
                                    minDistance = distance;
                                    self._closestView = view;
                                    self._closestEnd = { id: view.model.id };
                                }
                            }
                        }
                        ...
                        view.$('[magnet]').each(function(index, magnet) {
                            ...
                            var bbox = V(magnet).bbox(false, self.paper.viewport);
                            ...
                            if (distance < r && distance < minDistance) {
                                ...
                                if (self.paper.options.validateConnection.apply(
                                    self.paper, self._validateConnectionArgs(view, magnet)
                                )) {
                                    minDistance = distance;
                                    self._closestView = view;
                                    self._closestEnd = {
...
joint.dia.Paper = joint.mvc.View.extend({
    ...
    findViewsFromPoint: function(p) {
        ...
        var views = _.map(this.model.getElements(), this.findViewByModel.bind(this));

        var self = this;
        return _.filter(views, function(view) {
            return view && g.rect(view.vel.bbox(false, self.viewport)).containsPoint(p);
        });
    },
    ...
    findViewsInArea: function(rect, opt) {
        ...
        var self = this;
        return _.filter(views, function(view) {
            return view && rect[method](g.rect(view.vel.bbox(false, self.viewport)));
        });
    },
    ...
    PortData.prototype = {
        ...
        _getNormalizedGroups: function(data) {
            ...
            _.each(data.groups, function(group) {
                group.position = self._getPosition(group.position, true);
                group.label = self._getLabel(group, true);
            });
    ...
    _.extend(joint.dia.ElementView.prototype, {
        ...
        _renderPorts: function() {
            ...
            _.each(ports[withoutZKey], function(port) {
                var portElement = self._getPortElement(port);
            ...
            _.each(ports, function(groupPorts, groupName) {
                if (groupName !== withoutZKey) {
                    ...
                    self._appendPorts(ports[groupName], z, elementReferences);
                }
            });
        ...
        _appendPorts: function(ports, z, refs) {
            ...
            var portElements = _.map(ports, this._getPortElement.bind(this));
        ...
        _updatePorts: function() {
            ...
            _.each(_.groupBy(ports, 'group'), function(ports, groupName) {
                ...
                var group = self.model.portData.getGroup(groupName);
                _.each(ports, self._updatePortAttrs.bind(self));
                self._layoutPorts(ports, group, elBBox.clone());
            });
        },
        ...
        _layoutPorts: function(ports, group, elBBox) {
            ...
            var portTrans = namespace[position](_.map(ports, 'position.args'), elBBox, group.position.args || {});

            var self = this;
            _.each(portTrans, function(offset, index) {

                var port = self.model.portData.getPort(ports[index].id);
                var cached = self._portElementsCache[port.id] || {};

                self.applyPortTransform(cached.portElement, offset);
                ...
                if (namespace[labelPosition]) {
                    ...
                    self.applyPortTransform(cached.portLabelElement, labelTrans, -(offset.angle || 0));
                }
            });
        },
...
joint.routers.manhattan = (function(g, _, joint) {
    ...
    function findRoute(start, end, map, opt) {
        ...
        startPoints = _.filter(startPoints, map.isPointAccessible.bind(map));
        endPoints = _.filter(endPoints, map.isPointAccessible.bind(map));
...
(function(_, g, joint) {
    ...
    function lineLayout(ports, p1, p2) {
      ...
      var self = g.line(p1, p2);

        return _.map(ports, function(port, index, ports) {
            var p = self.pointAt(((index + 0.5) / ports.length));
